...
Z1$sdev
#Z1$loadings
Z1_scores = Z1$scores[,1:9]
new_td_data = cbind(td_data[,1] ,Z1_scores)
td_data = data.frame(new_td_data)
colnames(td_data) = c("Total.damaged.houses..rel..", "x1","x2","x3","x4","x5","x6","x7","x8","x9")
...
Z2$sdev
#Z1$loadings
Z2_scores = Z2$scores[,1:9]
new_pc_data= cbind(pc_data[,1] ,Z2_scores)
pc_data = data.frame(new_pc_data)
colnames(pc_data) = c("Completely.damaged..rel..", "y1","y2","y3","y4","y5","y6","y7","y8","y9")
# # 将数据分别写入文件td_data和pc_data
# write.csv(td_data, file="data//1//td_data1.csv")
# write.csv(pc_data, file="data//1//pc_data1.csv")
#----------------------------------PCA
...
for (i in 1:40){
  ...
  if(include_transform){
    ...
    colnames(td_data.i) = c("Total.damaged.houses..rel..", "x1","x2","x3","x4","x5","x6","x7","x8","x9" )
    ...
    colnames(pc_data.i) = c("Completely.damaged..rel..", "y1","y2","y3","y4","y5","y6","y7","y8","y9")
    # # 将数据分别写入文件td_data和pc_data
    # write.csv(td_data, file="data//1//td_data1.csv")
    # write.csv(pc_data, file="data//1//pc_data1.csv")
    #----------------------------------PCA
...
write.csv(cv_pc, 'data\\B1PCA_randomforest_pc.csv', row.names=F)
write.csv(cv_td, 'data\\B1PCA_randomforest_td.csv', row.names=F)
