...
X <- td_data[,2:14]  
Y <- pc_data[,2:14] 
factor<- factanal(x=X, factor=7, scores="regression")
#查看贡献度
#根据选出来的因子数为7
Z1_scores = factor$scores[,1:7]
Z1_scores = Z1$scores[,1:7]
new_td_data = cbind(td_data[,1] ,Z1_scores)
td_data = data.frame(new_td_data)
colnames(td_data) = c("Total.damaged.houses..rel..", "x1","x2","x3","x4","x5","x6","x7")

factor <- factanal(x=Y, factor=7, scores="regression")

Z2_scores = factor$scores[,1:7]
new_pc_data= cbind(pc_data[,1] ,Z2_scores)
pc_data = data.frame(new_pc_data)
colnames(pc_data) = c("Completely.damaged..rel..", "y1","y2","y3","y4","y5","y6","y7")
...
for (i in 1:40){
  ...
  if(include_transform){
    ...
    colnames(td_data.i) = c("Total.damaged.houses..rel..", "x1","x2","x3","x4","x5","x6","x7" )
    ...
    colnames(pc_data.i) = c("Completely.damaged..rel..", "y1","y2","y3","y4","y5","y6","y7")
...
write.csv(cv_pc, 'data\\B3PSY_randomforest_pc.csv', row.names=F)
write.csv(cv_td, 'data\\B3PSY_randomforest_td.csv', row.names=F)
