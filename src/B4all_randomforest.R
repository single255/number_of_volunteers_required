...
# omit_var_td_data <- which(colnames(td_data) %in% c("rainfall", "Slope", "slope_stdev", "Population.density", "X..skilled.Agriculture.Forestry.Fishermen", "Experience"))
# td_data <- td_data[, -omit_var_td]
# # Omit "rainfall", "Slope", "slope_stdev", "Population.density", "X..skilled.Agriculture.Forestry.Fishermen", "Experience"
# omit_var_pc_data <- which(colnames(pc_data) %in% c("rainfall", "Slope", "slope_stdev", "Population.density", "X..skilled.Agriculture.Forestry.Fishermen", "Experience"))
# pc_data <- pc_data[, -omit_var_pc]
# 
# #----------------------------------glmnet
...
for (i in 1:40){
  ...
  if(include_transform){
    ...
    # #----------------------------------glmnet
    # # Omit Population, Land.Area, Region and prov.
    # omit_var_td_data.i <- which(colnames(td_data.i) %in% c("rainfall", "Slope", "slope_stdev", "Population.density", "X..skilled.Agriculture.Forestry.Fishermen", "Experience"))
    # td_data.i <- td_data.i[, -omit_var_td_data.i]
    # # Omit Population, Land.Area, Region and prov.
    # omit_var_pc_data.i <- which(colnames(pc_data.i) %in% c("rainfall", "Slope", "slope_stdev", "Population.density", "X..skilled.Agriculture.Forestry.Fishermen", "Experience"))
...
write.csv(cv_pc, 'data\\B4all_randomforest_pc.csv', row.names=F)
write.csv(cv_td, 'data\\B4all_randomforest_td.csv', row.names=F)
e=Sys.time()
print(e-s) 
# #残差分析
...
cap.df <- data.frame(x = 126.8, y = 15.75, text = "")
...
ggplot() + theme(
  ...
  scale_fill_fermenter(breaks = c(0,0.2,0.4,0.6,0.8)*10000, type = "seq", palette = 7, direction=1,
                       na.value='grey50') + 
  geom_sf(data = goni_trace, size = 0.2, color = "red", fill = "cyan1") + 
  coord_sf(xlim=c(118,127), ylim=c(11,16), expand=F) + labs(fill = "Number of Volunteers", x = "Longitude", y = "latitude") + geom_text(data = cap.df, aes(x = x, y = y, label = text), size = 2.5, hjust = 1) +
  theme(legend.position =  c(0.025, 0.15), legend.justification = c(0, 0), legend.title = element_text(size = 7.5), legend.text = element_text(size = 7.5))

ggsave("graphics/goni-damagedhouses.png", dpi = 3000, height = 6, width = 20, units = "in")
