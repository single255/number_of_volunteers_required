...
#----------------------------------glm
# Omit "rainfall", "Slope", "slope_stdev", "Population.density", "X..skilled.Agriculture.Forestry.Fishermen", "Experience"
omit_var_td_data <- which(colnames(td_data) %in% c("Distance.to.typhoon", "rainfall",  "slope_stdev", "Poverty.incidence", "X..strong.wall.type", "X..skilled.Agriculture.Forestry.Fishermen"))
td_data <- td_data[, -omit_var_td_data]
# Omit "rainfall", "Slope", "slope_stdev", "Population.density", "X..skilled.Agriculture.Forestry.Fishermen", "Experience"
omit_var_pc_data <- which(colnames(pc_data) %in% c("Distance.to.typhoon", "rainfall", "Slope", "slope_stdev", "Poverty.incidence","X..strong.wall.type", "X..skilled.Agriculture.Forestry.Fishermen"))
pc_data <- pc_data[, -omit_var_pc_data]
...
# td_full <- randomForest(Total.damaged.houses..rel.. ~ ., data= td_data, importance=TRUE, proximity=TRUE)
# 
...
# pc_full <- randomForest(Completely.damaged..rel.. ~ ., data= pc_data, importance=TRUE, proximity=TRUE)
# ## Check on variable collinearity
# temp_model <- pc_full
# temp_vif <- vif(temp_model) # Use a threshold of 10 to remove variables.
temp_var <- "dummy"
...
for (i in 1:40){
  ...
  if(include_transform){
    ...
    #----------------------------------glm
    # Omit "rainfall", "Slope", "slope_stdev", "Population.density", "X..skilled.Agriculture.Forestry.Fishermen", "Experience"
    omit_var_td_data.i <- which(colnames(td_data.i) %in% c("Distance.to.typhoon", "rainfall",  "slope_stdev", "Poverty.incidence", "X..strong.wall.type", "X..skilled.Agriculture.Forestry.Fishermen"))
    td_data.i <- td_data.i[, -omit_var_td_data.i]
    # Omit "rainfall", "Slope", "slope_stdev", "Population.density", "X..skilled.Agriculture.Forestry.Fishermen", "Experience"
    omit_var_pc_data.i <- which(colnames(pc_data.i) %in% c("Distance.to.typhoon", "rainfall", "Slope", "slope_stdev", "Poverty.incidence","X..strong.wall.type", "X..skilled.Agriculture.Forestry.Fishermen"))
    pc_data.i <- pc_data.i[, -omit_var_pc_data.i]
    #----------------------------------glm
    ...
    td_full.i <- randomForest(Total.damaged.houses..rel.. ~ ., data= td_data.i, importance=TRUE, proximity=TRUE)
    ## Check on variable collinearity
    ...
    pc_full.i <- randomForest(Completely.damaged..rel.. ~ ., data= pc_data.i, importance=TRUE, proximity=TRUE)
    ## Check on variable collinearity
    ...
    pc_full.i <- randomForest(Completely.damaged..rel.. ~ ., data= pc.training.data.i, importance=TRUE, proximity=TRUE)
    ...
    td_full.i <- randomForest(Total.damaged.houses..rel.. ~ ., data= td.training.data.i, importance=TRUE, proximity=TRUE)
    ...
    pc_model.i <- randomForest(Completely.damaged..rel.. ~ ., data= pc.training.data.i, importance=TRUE, proximity=TRUE)
    ...
    td_model.i <- randomForest(Total.damaged.houses..rel.. ~ ., data= td.training.data.i, importance=TRUE, proximity=TRUE)
  }
...
write.csv(cv_pc, 'data\\B6glm_randomforest_pc.csv', row.names=F)
write.csv(cv_td, 'data\\B6glm_randomforest_td.csv', row.names=F)
# #残差分析
